...
class CombinedLoss(nn.Module):
    def __init__(self, l1_weight, dolp_weight):
        super(CombinedLoss, self).__init__()
        self.l1_loss = nn.L1Loss()
        self.l1_weight = l1_weight
        self.dolp_weight = dolp_weight
    
    def calculate_dolp(self, img):
    ...
    def forward(self, pred, target):
        ...
        pred_dolp = self.calculate_dolp(pred)
        target_dolp = self.calculate_dolp(target)
        
        
        dolp_loss = self.l1_loss(pred_dolp, target_dolp)
        
        
        total_loss = self.l1_weight * l1_loss + self.dolp_weight * dolp_loss
        
        return total_loss, l1_loss, dolp_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--l1-weight', type=float, default=0.8, help='Weight for L1 loss')
    parser.add_argument('--dolp-weight', type=float, default=0.2, help='Weight for DOLP loss')
    ...
    criterion = CombinedLoss(l1_weight=args.l1_weight, dolp_weight=args.dolp_weight)
    ...
    for epoch in range(args.num_epochs):
        ...
        epoch_dolp_losses = AverageMeter()
        ...
        with tqdm(total=(len(train_dataset) - len(train_dataset) % args.batch_size), ncols=120) as t:
            ...
            for data in train_dataloader:
                ...
                total_loss, l1_loss, dolp_loss = criterion(preds, labels)

                epoch_losses.update(total_loss.item(), len(inputs))
                epoch_l1_losses.update(l1_loss.item(), len(inputs))
                epoch_dolp_losses.update(dolp_loss.item(), len(inputs))
                ...
                t.set_postfix(totaloss='{:.6f}'.format(epoch_losses.avg), 
                             ...
                             dolp='{:.6f}'.format(epoch_dolp_losses.avg))
                t.update(len(inputs))
